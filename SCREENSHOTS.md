...
| 파일명 | 무엇을 찍나 | 사용 위치 |
|--------|------------|----------|
| `IDE_MODEL_ADD.png` | **독립 실행(BYOK) 모드**의 설정 → AI 모델 탭 — 왼쪽 메뉴에서 AI 모델을 선택한 화면에서 **사용자 정의 모델** 영역과 `+ 모델 추가` 버튼이 보이게. (이 문서는 "AI 모델 탭 열기"와 "모델 추가"를 한 스텝으로 합쳤으므로 탭 전체가 나오도록 촬영) | AI 모델 연결 |
...
> **참고 — 선택 사항:** 기존 `IDE_AI_MODEL.png`(조직 로그인 화면, `ide/settings.mdx` 에서 사용)는 그대로 써도 되지만, 화면에 `Gemini 3.0 Flash` 가 보입니다. 신경 쓰이면 최신 화면으로 다시 찍어 같은 파일명으로 덮어쓰세요. **BYOK 화면이 아니라 조직 로그인 화면이어야 합니다.**
