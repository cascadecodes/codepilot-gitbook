...
이 저장소는 **공개(public)** 이고, git 히스토리는 나중에 이미지를 바꿔도 남습니다. 촬영 시 화면에 무엇이 함께 찍히는지 확인하세요.

| 파일명 | 상태 | 어떻게 다시 찍나 |
|--------|------|-----------------|
| `IDE_INSTALL.png` | **커밋 보류** — 파일 선택 대화상자에 고객사 라이선스 파일명(`license-<고객사>-<날짜>.key`)과 AWS 자격증명 관련 파일이 함께 보입니다 | `.vsix` 파일 **하나만** 있는 빈 폴더에서 촬영 |
