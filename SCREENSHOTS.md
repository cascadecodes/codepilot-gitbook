...
## 재촬영이 필요한 이미지 — 커밋 보류 중

이 저장소는 **공개(public)** 입니다. 아래 두 장은 외부에 노출되면 안 되는 정보가 찍혀 있어 커밋하지 않았습니다.

| 파일명 | 무엇이 노출되나 | 어떻게 다시 찍나 |
|--------|----------------|-----------------|
| `IDE_INSTALL.png` | 파일 선택 대화상자에 **고객사 라이선스 파일명**(`license-<고객사>-<날짜>.key`)과 AWS 자격증명 관련 파일이 함께 보입니다 | `.vsix` 파일 **하나만** 있는 빈 폴더에서 촬영 |
| `IDE_FIRST_RUN.png` | 터미널의 내부 빌드 경로·코어 바이너리명, 디버거 패널, 타사 확장, 상태바 모델명 | 터미널·디버거 닫고 패널 중심으로 촬영 |
